...
func getData(serialNo string, offset string, limit string) []Weather {
	var weatherDeatils []Weather
	intPageSize, err := strconv.Atoi(limit)
	...
	cursor, err := collection.Find(context.Background(), bson.NewDocument(bson.EC.String("serialNo", serialNo)), findopt.Skip(int64(intOffset)), findopt.Limit(int64(intPageSize)))
	if err != nil {
		fmt.Println(err)
	}
